  ...
  return (
    <div id="screen-counselor-sheet-management" className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <h2 className="mt-1 text-xl font-bold">Quản lý tư vấn viên và tài khoản</h2>
              <p className="mt-1 text-xs text-blue-100">
                Chỉnh hồ sơ tại tab counselors và tài khoản đăng nhập tại tab users.
              </p>
            </div>
          </div>
      ...
      <div className="flex items-start gap-2 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-xs text-emerald-900">
        ...
        <span>
          Email ở tab <strong>users</strong> phải trùng email Counselor, hoặc điền <strong>counselor_id</strong>.
          Đặt trạng thái tài khoản là <strong>ACTIVE</strong> để đăng nhập.
        </span>
      </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">{counselors.length} hồ sơ đang hoạt động</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-12 text-center text-slate-500">Chưa có tư vấn viên phù hợp.</td></tr>
                ...
                <tr key={counselor.id} className="hover:bg-blue-50/40">
                  ...
                  <td className="px-4 py-3"><span className={`rounded-full px-2.5 py-1 font-bold ${counselor.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{counselor.status || 'ACTIVE'}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
